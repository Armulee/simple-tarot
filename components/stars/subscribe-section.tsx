...
export default function SubscribeSection({
    ...
    useEffect(() => {
        ...
        async function fetchOfferEligibility() {
            try {
                ...
                console.error("Error checking subscription offer:", err)
                setHasUsedOffer(true)
            } finally {
                setOfferLoading(false)
            }
    ...
    const isEligibleForOffer =
        !offerLoading && Boolean(user) && !hasUsedOffer && !hasActiveInfinity
    const monthlyCouponId =
        isEligibleForOffer && plan === "monthly" ? "DX9hJoW7" : undefined
