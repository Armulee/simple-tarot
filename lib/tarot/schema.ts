...
export const tarotInterpretationSchema = z.object({
    ...
    detailedHtml: z
        .string()
        .describe(
            "A substantive, scannable HTML block of key takeaways for the reading, written in the SAME language as the user's question. Should read like a thoughtful assistant response — depth comparable to ~180-260 English words (or proportional in Thai/Lao/etc.), enough to feel like a real interpretation, not a glossary. Renders directly BELOW the `headline`/`subtitle` and ABOVE the per-card breakdown, so it MUST NOT contain its own headline (the headline field already plays that role). REQUIRED STRUCTURE: (1) an opening <p> of 2-4 sentences that grounds the reading — sets the emotional/practical context, frames the situation, and bridges into the takeaways (NOT a restatement of headline/subtitle); then (2) ONE <ul> with 3-5 <li> bullets, where each bullet starts with a 2-4-word lead phrase wrapped in <strong>…</strong> followed by ': ' and a substantive takeaway of 1-3 sentences (target ~25-55 words per bullet, longer for Thai/Lao). Vary bullet length naturally — some terse, some richer — so the block reads human, not templated. Optionally, end with ONE closing <p> of 1-2 sentences that ties the takeaways together with warmth or a gentle prompt to act (omit if it would feel redundant). Each bullet must cover a DIFFERENT angle (e.g. inner state, external dynamic, opportunity, watch-out, suggested move, timing) and translate the card energy into the question's specific domain — concrete, not generic spiritual language. Use <span class=\"highlight-gold\">…</span> on 1-3 of the most must-not-miss short phrases across the whole block (date, decision, action, warning) — never wrap a whole bullet or paragraph. Use <em> sparingly for soft emphasis or evocative phrasing. ALLOWED TAGS ONLY: <p>, <strong>, <em>, <ul>, <li>, <br>, and <span class=\"highlight-gold\">. FORBIDDEN TAGS: <h1>-<h6>, <ol>, <b>, <i>, and anything not listed. Do NOT emit more than one list. Do NOT include attributes, inline styles, classes (other than highlight-gold), scripts, links, or images. Do NOT wrap output in <html>, <body>, code fences, or Markdown. Do NOT mention card names. Apply the same tendency/leaning tone as the rest of the reading — never absolute verdicts.",
        ),
    perCard: z
        .array(
            z.object({
