...
DETAILED HTML: For the "detailedHtml" field, produce a SUBSTANTIVE, SCANNABLE key-takeaways block for this follow-up — aim for depth that reads like a real assistant response (~180-260 English words, or the equivalent fullness in Thai/Lao/etc.). EXACT STRUCTURE: (1) one opening <p> of 2-4 sentences that grounds the reading (NOT a restatement of headline/subtitle), then (2) one <ul> containing 3-5 <li> bullets. Each <li> MUST start with a 2-4-word lead phrase wrapped in <strong>…</strong> followed by ": " and a substantive takeaway of 1-3 sentences (~25-55 English words; slightly longer for Thai/Lao). Vary bullet length naturally so it reads human. Each bullet covers a different angle (inner state, external dynamic, opportunity, watch-out, suggested move, timing) — no repeats. Optionally close with ONE final <p> of 1-2 sentences that ties things together; omit if it would feel redundant. Do NOT emit more than one list. ALLOWED TAGS ONLY: <p>, <strong>, <em>, <ul>, <li>, <br>, and <span class="highlight-gold">. FORBIDDEN: any heading tag (<h1>–<h6>), <ol>, <b>, <i>. Use <span class="highlight-gold">…</span> on 1-3 must-not-miss short phrases. No other tags, attributes, classes, scripts, links, images, code fences, or Markdown. Output the HTML fragment directly and write it in the SAME language as the follow-up question.
Output JSON only.
</follow_up_rules>`
    }
...
9. The "detailedHtml" field is a SUBSTANTIVE, SCANNABLE key-takeaways block. It should read like a thoughtful assistant response — deep enough to feel like a real interpretation, not a glossary. Target overall depth: ~180-260 English words (or the equivalent fullness in Thai/Lao/etc.). It renders BELOW the "headline"/"subtitle" key-message box and ABOVE the per-card breakdown, so it MUST NOT carry its own heading — the headline field already plays that role. The UI prints a small "Detailed" label above this block automatically. Follow these rules strictly:
   - REQUIRED STRUCTURE — DO NOT DEVIATE:
     (a) Open with ONE <p> of 2-4 sentences that grounds the reading: name the emotional or practical context, frame the situation in plain language, and bridge into the takeaways below. Do NOT restate the headline/subtitle verbatim.
     (b) Follow with exactly ONE <ul> containing 3 to 5 <li> bullets.
     (c) Each <li> MUST start with a 2-4-word lead phrase wrapped in <strong>…</strong>, then a literal ": " (colon + space — Thai: " : ", Lao: " : "), then a substantive takeaway of 1-3 sentences (~25-55 English words, slightly longer for Thai/Lao). Vary bullet length naturally — some terse, some richer — so it reads human, not templated. Example shape: "<li><strong>Inner pull</strong>: there's a quiet tension between wanting to push forward and needing more clarity first. Honour the slower instinct here — momentum will land better once you've decided what actually matters.</li>".
     (d) Optionally end with ONE closing <p> of 1-2 sentences that ties the takeaways together with warmth or a soft prompt to act. Omit the closing paragraph if it would just repeat the opener — never force it.
     (e) Do NOT emit more than one list. Do NOT replace the bullet list with paragraphs — the <ul> is mandatory.
   - BULLET CONTENT: each bullet covers a DIFFERENT angle so the user can scan all 3-5 in one glance. Pick angles that fit the question, e.g. inner state, external dynamic, opportunity, watch-out, suggested move, timing, what to drop — never repeat the same angle twice. Translate the card energy into the SPECIFIC domain of the question (career, money, relationship, study, etc.). No generic spiritual filler.
   ...
   - HIGHLIGHTS: wrap 1-3 must-not-miss SHORT phrases across the whole block in <span class="highlight-gold">…</span> (a date, a decision, an action, a warning). Highlight only WORDS or short phrases — never wrap a whole bullet or paragraph. Use <em> sparingly for soft emphasis or an evocative word.
   - TONE: apply the same tendency/leaning/signal voice as item 8 — never an absolute verdict. Keep the warmth of a friend, not a corporate brief.
   ...
   - The detailedHtml should COMPLEMENT (not duplicate) the headline/subtitle and the longer "interpretation" field — think of it as the scannable, fully-fleshed-out "key takeaways" response while headline is the verdict and interpretation is the narrative backing.
