...
1. **Poster** — `NEXT_PUBLIC_AVATAR_POSTER`, default `/avatar/astra-idle.webp`. Always present, paints immediately, and is the LCP element. A missing file degrades to the gradient rather than a broken image.
...
NEXT_PUBLIC_AVATAR_POSTER=/avatar/astra-idle.webp # idle still (default)
