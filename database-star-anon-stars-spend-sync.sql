-- Patch: keep anon_stars in sync with daily_stars for anonymous rows (fixes ok:false spend after migration).
-- Run in Supabase SQL editor if stars revert after asking a question.
      ...
      if v_sync is distinct from v_state.daily_stars
        ...
        update public.stars s
           set daily_stars = v_sync,
               anon_stars = v_sync,
               anon_last_refill_at = coalesce(s.anon_last_refill_at, v_state.daily_last_refill_at),
