...
function main(){
  ...
  // English is the canonical source. Write the root copy + en/ only — never
  // overwrite translated locale folders. Run scripts/translate-meanings.mjs to
  // (re)generate the other locales from en/.
  const enDir = path.join(OUTPUT_DIR, 'en');
  if (!fs.existsSync(enDir)) fs.mkdirSync(enDir, { recursive: true });
  for (const [slug, data] of Object.entries(generated)) {
    const json = JSON.stringify(data, null, 2);
    fs.writeFileSync(path.join(OUTPUT_DIR, `${slug}.json`), json);
    fs.writeFileSync(path.join(enDir, `${slug}.json`), json);
  }
  console.log('Wrote English meanings to', OUTPUT_DIR, 'and en/. Run translate-meanings.mjs for other locales.');
}
